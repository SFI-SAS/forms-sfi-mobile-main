...
const FirmField = ({
  ...
  // 🔥 Configurar listener de deep linking para recibir respuesta de firma
  useEffect(() => {
  ...
  // 🔥 Manejar deep link de respuesta de firma
  const handleDeepLink = ({ url }) => {
    ...
    try {
      ...
      if (queryParams?.firmData) {
        // Decodificar datos de firma
        const firmData = JSON.parse(decodeURIComponent(queryParams.firmData));
        console.log("✅ Datos de firma recibidos:", firmData);

        handleSignSuccess(firmData);
      } else if (queryParams?.error) {
        console.error("❌ Error en firma:", queryParams.error);
        setFirmError(decodeURIComponent(queryParams.error));
        setAuthStatus("error");
      }
    } catch (error) {
  ...
  const handleFirmar = async () => {
    ...
    // 🆕 Si estamos OFFLINE y NO hay firma guardada, mostrar error
    if (isOffline) {
      Alert.alert(
        "Sin conexión",
        "No es posible realizar firma digital sin conexión a internet. La firma requiere validación en línea."
      );
      return;
    }
    ...
    try {
      ...
      const redirectUrl = Linking.createURL("signature-callback");
      const signatureUrl = `https://reconocimiento-facial-safe.service.saferut.com/sign.html?apiUrl=${encodeURIComponent(apiUrl)}&redirectUrl=${encodeURIComponent(redirectUrl)}&userId=${encodeURIComponent(selectedUser.id)}&documentHash=${encodeURIComponent(documentHash)}`;

      console.log("🌐 Abriendo navegador para firma...");
      console.log("📍 Signature URL:", signatureUrl);
      console.log("🔙 Redirect URL:", redirectUrl);

      // Abrir navegador
      const result = await WebBrowser.openBrowserAsync(signatureUrl, {
        dismissButtonStyle: "close",
        presentationStyle: WebBrowser.WebBrowserPresentationStyle.PAGE_SHEET,
        controlsColor: "#0F8593",
      });

      console.log("🔵 Browser result:", result);

      // Si el usuario cerró el navegador manualmente
      if (result.type === "cancel") {
        console.log("❌ Usuario canceló la firma");
        setAuthStatus("idle");
      }
    } catch (e) {
      console.error("❌ Error abriendo navegador:", e);
      onFirmError?.(e);
      Alert.alert("Error", "No se pudo abrir el navegador de firma digital.");
      setAuthStatus("error");
      setFirmError("No se pudo abrir el navegador");
    } finally {
